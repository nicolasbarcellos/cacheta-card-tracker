...
def test_qualquer_tamanho_estavel_e_aceito():
    """O leitor nao sabe que jogo e este: poquer tem 2 cartas, truco 3.

    Ate 2026-08-19 so passavam `hand_size` e `hand_size + 1`, e uma leitura
    estavel de 7 era recusada como "incompleta". Isso deixou de valer quando o
    mesmo modelo passou a servir varias modalidades — nao existe "incompleta"
    sem saber o tamanho certo.

    Contra a bagunca da transicao sobrou a estabilidade: leitura que oscila nao
    dura `lock_frames`, e carta que pisca nao chega a sair do score (ver
    `test_flickering_ninth_card_holds_locked` e `test_brief_dropout_does_not_unlock`).
    """
    sh = StableHand(lock_frames=12)
    feed(sh, NINE, 20)
    assert len(sh.cards) == 9
    feed(sh, NINE[:7], 40)                   # 7 cartas, mas ESTAVEL
    assert sorted(sh.cards) == sorted(NINE[:7])
